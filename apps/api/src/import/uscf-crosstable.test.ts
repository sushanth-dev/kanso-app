...
1,"Kamabathula, Sushanth",1450,1.5,W-2-1,B-3-0.5
...
describe('parseCrosstable', () => {
  test('parses players and their round cells, ignoring the header', () => {
    ...
    expect(players[0]).toMatchObject({ name: 'Kamabathula, Sushanth', rating: 1450 });
...
describe('synthesizeGames', () => {
  ...
  test('builds header-only games for the matching player', () => {
    const result = synthesizeGames(players, 'Sushanth Kamabathula', 'Event', 'tid', 20);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.games).toHaveLength(2);
    expect(result.games[0]!.externalId).toBe('tid:1');
    expect(result.games[0]!.pgn).toContain('[White "Kamabathula, Sushanth"]');
    ...
    expect(result.games[1]!.pgn).toContain('[Black "Kamabathula, Sushanth"]');
  ...
  test('refuses a namesake whose surname matches but name does not', () => {
    const withNamesake = parseCrosstable(
      `rank,name,rating,total,R1\n1,"Kamabathula, Aarav",1450,1.0,W-2-1`,
    );
    const result = synthesizeGames(withNamesake, 'Sushanth Kamabathula', 'Event', 'tid', 20);
  ...
  test('refuses two exact matches as ambiguous', () => {
    const duplicate = parseCrosstable(
      `rank,name,rating,total,R1\n1,"Kamabathula, Sushanth",1450,0.0,W-2-1\n2,"Sushanth Kamabathula",1500,1.0,B-1-0`,
    );
    const result = synthesizeGames(duplicate, 'Sushanth Kamabathula', 'Event', 'tid', 20);
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  test('caps the games at maxGames', () => {
    const result = synthesizeGames(players, 'Sushanth Kamabathula', 'Event', 'tid', 1);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
