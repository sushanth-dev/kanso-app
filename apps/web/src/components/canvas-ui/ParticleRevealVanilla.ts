...
export interface ParticleRevealOptions {
  ...
  fade?: number;
  /** Contrast above which a pixel counts as UI and dissolves into dust. */
  threshold?: number;
  /** Color of the backdrop behind the content, as any CSS color. */
  background?: string;
...
// Minimal faithful version of canvasui.dev's rect-cache: the bounding rect is
// cached and invalidated on resize and scroll, so pointer math does not thrash
// layout on every move.
