 ...
 * `body` restates the boundary when the surface is not the diagnosis loop
 * (ST-111: the coach's monthly budget); the default keeps the loop copy.
 */
export function UpgradePrompt({
  title,
  body = 'Your first diagnosis is free. A focus, verification afterwards, and the proof sheet you send a parent are paid.',
}: {
  title: string;
  body?: string;
}) {
